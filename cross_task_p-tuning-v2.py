...
from sequence_classification import RobertaPrefixForSequenceClassification
...
def test():
    ...
    for ids, l in tq(sst_test_loader):
        ids = ids.to(device)
        with torch.no_grad():
            logit = model(ids)[0].cpu().numpy().tolist()
    ...
    if all_acc > best_all_acc:
        torch.save(model.state_dict(), "/home/hadoop-mtai/cephfs/data/xiajinxiong/workspace/Transfer/saved_model"
                                       "/cross_task_roberta_p-tuning_v2.pt", _use_new_zipfile_serialization=False)
        print("saving p-tuning-v2 to /home/hadoop-mtai/cephfs/data/xiajinxiong/workspace/Transfer/saved_model"
              "/cross_task_roberta_p-tuning_v2.pt")
...
config = RobertaConfig.from_pretrained(num_labels=2, pretrained_model_name_or_path=model_name)
